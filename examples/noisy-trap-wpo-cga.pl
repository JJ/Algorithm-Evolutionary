...
use Algorithm::Evolutionary::Op::Canonical_GA_NN;
...
my $generation = Algorithm::Evolutionary::Op::Canonical_GA_NN->new(  $selection_rate , [$m, $c] ) ;
...
do {
    ...
    my @sorted_pop = rnkeysort { $_->Fitness() } @pop;
    say "Generación $contador  ";
    $contador++;
    $best_found = ( $sorted_pop[0]->{'_str'} eq $best );
    say "\t", $sorted_pop[0]->asString();
    $best_guy = $sorted_pop[0]; # Provisional value
    my $i = 1;
    while ( $sorted_pop[$i]->Fitness() == $sorted_pop[0]->Fitness() && !$best_found ) {
      if (  $sorted_pop[$i]->{'_str'} eq $best ) {
	$best_guy = $sorted_pop[$i];
	$best_found = 1;
      } else {
	say "\t", $sorted_pop[$i]->asString();
	$i++;
      }
    }
    if ( !$best_found) {
